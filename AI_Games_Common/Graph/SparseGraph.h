...
#include <string>
#include <iostream>
//http://stackoverflow.com/questions/4861777/missing-stdruntime-error-in-qtmingw
#include <exception>
#include <stdexcept>
...
public:
  ...
  int    NumEdges()const
  {
    ...
    typedef typename EdgeListVector::const_iterator EIterator;
    for (EIterator curEdge = m_Edges.begin();
         curEdge != m_Edges.end();
         ++curEdge)
    {
      tot += curEdge->size();
  ...
  void RemoveEdges()
  {
    typedef typename EdgeListVector::iterator EIterator;
    for (EIterator it = m_Edges.begin(); it != m_Edges.end(); ++it)
    {
      it->clear();
    }
  }
...
bool SparseGraph<node_type, edge_type>::isEdgePresent(int from, int to)const
{
    ...
    {
       typedef typename EdgeList::const_iterator EIterator;
        for (EIterator curEdge = m_Edges[from].begin();
...
{
  typedef typename EdgeListVector::iterator EVIterator;
  typedef typename EdgeList::iterator       EIterator;

  for (EVIterator curEdgeList = m_Edges.begin(); curEdgeList != m_Edges.end(); ++curEdgeList)
  {
    for (EIterator curEdge = (*curEdgeList).begin(); curEdge != (*curEdgeList).end(); ++curEdge)
    {
...
void SparseGraph<node_type, edge_type>::RemoveNode(int node)
{
  ...
  if (!m_bDigraph)
  {
    ...
    typedef typename EdgeList::iterator EIterator;
    for (EIterator curEdge = m_Edges[node].begin();
         curEdge != m_Edges[node].end();
         ++curEdge)
    {
      for (EIterator curE = m_Edges[curEdge->To()].begin();
...
bool SparseGraph<node_type, edge_type>::Save(std::ofstream& stream)const
{
  ...
  typedef typename NodeVector::const_iterator NIterator;
  NIterator curNode = m_Nodes.begin();
  ...
  {
    typedef typename EdgeList::const_iterator EIterator;
    for (EIterator curEdge = m_Edges[nodeIdx].begin();
