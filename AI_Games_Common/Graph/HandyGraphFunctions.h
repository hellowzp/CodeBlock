    ...
    for (int i=-1; i<2; ++i)
    {
        for (int j=-1; j<2; ++j)
        {
            ...
            if (ValidNeighbour(nodeX, nodeY, NumCellsX, NumCellsY))
            {
                ...
                typedef typename graph_type::EdgeType GEdge;
                GEdge NewEdge(row*NumCellsX+col,
                ...
                {
                    typedef typename graph_type::EdgeType GEdge;
                    GEdge NewEdge(nodeY*NumCellsX+nodeX,
                                  row*NumCellsX+col,
                                  dist);
                    graph.AddEdge(NewEdge);
                }
...
void GraphHelper_DrawUsingGDI(const graph_type& graph, int color, bool DrawNodeIDs = false)
{
    ...
    //draw the nodes
    typedef typename graph_type::ConstNodeIterator GNIterator;
    typedef typename graph_type::ConstEdgeIterator GEIterator;
    typedef typename graph_type::NodeType GNode;
    typedef typename graph_type::EdgeType GEdge;

    GNIterator NodeItr(graph);
    for (const GNode* pN=NodeItr.begin();
            !NodeItr.end();
            pN=NodeItr.next())
    {
        gdi->Circle(pN->Pos(), 2);
        ...
        GEIterator EdgeItr(graph, pN->Index());
        for (const GEdge* pE=EdgeItr.begin();
                !EdgeItr.end();
                pE=EdgeItr.next())
        {
...
void WeightNavGraphNodeEdges(graph_type& graph, int node, double weight)
{
    ...
    GEIterator ConstEdgeItr(graph, node);
    for (const GEdge* pE=ConstEdgeItr.begin();
            !ConstEdgeItr.end();
...
std::vector<std::vector<int> > CreateAllPairsTable(const graph_type& G)
{
    ...
    for (int source=0; source<G.NumNodes(); ++source)
    {
        ...
        typedef typename graph_type::EdgeType GEdge;
        std::vector<const GEdge*> spt = search.GetSPT();
...
double CalculateAverageGraphEdgeLength(const graph_type& G)
{
    ...
    GNIterator NodeItr(G);
    const GNode* pN;
    for (pN = NodeItr.begin(); !NodeItr.end(); pN=NodeItr.next())
    {
        GEIterator EdgeItr(G, pN->Index());
        for (const GEdge* pE = EdgeItr.begin(); !EdgeItr.end(); pE=EdgeItr.next())
        {
