...
bool Graph_SearchDFS<graph_type>::Search()
{
  ...
  while (!stack.empty())
  {
    ...
    //visited node)
    typedef typename graph_type::ConstEdgeIterator GIterator;
    GIterator ConstEdgeItr(m_Graph, Next->To());
...
void Graph_SearchDijkstra<graph_type>::Search()
{
  ...
  while(!pq.empty())
  {
    ...
    typedef typename graph_type::ConstEdgeIterator GIterator;
    GIterator ConstEdgeItr(m_Graph, NextClosestNode);
...
private:
  ...
  void Search(const int source)
  {
    ...
    while(!pq.empty())
    {
      ...
      typedef typename graph_type::ConstEdgeIterator GIterator;
      GIterator ConstEdgeItr(m_Graph, best);
