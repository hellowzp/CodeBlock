...
int xbee_txReq_data_assemble(String mac_addr,String ntwk_addr,Byte radius,Byte option,String RFData,int len,String* retStr);
int xbee_txReqst_frame_assemble(String mac_addr,String ntwk_addr,Byte radius,Byte option,String RFData,int len);
...
int xbee_remoteAT_frame_assemble(String mac_addr,String ntwk_addr,Byte option,String cmd, Byte para);
int xbee_localAT_frame_assemble(String cmd,Byte value);
int xbee_mqtt_Wframe_assenble(mqtt_ptr_t mqtt);
int xbee_frame_escape(int len);  //length of buffer(outbuf)
...
String wframe_get_data_at_del( String pkt, int len, int n, char del);
String wframe_get_data_between_del( String pkt, int len, int bn, int en, char del);  
int wframe_assemble(Byte frame_type, Byte fields, String serial_id,Byte seq,map_ptr_t payload,String* wframe);
...
int int_to_str(int num, String* str);
String hex_to_str(String hexstr,int len);
int str_length(String str);
long power(Byte a, Byte b);
int string_to_int(String s, int base);
String str_get_between_index(String str, int bn, int en); //the return length can be get by en-bn+1
//concatenate variable length of strings(sensor field)
//if allow empty field, treat NULL field as empty and also add it
//else exit with error
String str_multi_cat(int num,  char del, ...);
int str_split(String str, char del,String** retArray);
map_ptr_t str_to_map(String str, char fdel, char sdel);
int map_cat(map_ptr_t map,  char fdel,  char sdel, String* str);
...
int xbee_localAT_frame_assemble(String cmd, Byte para) {
...
int xbee_mqtt_Wframe_assenble(mqtt_ptr_t mqtt) {
	...
	int len = mqtt->leng + 14;  //LENGTH FIELD not the total length
    outbuf[0] = ZIGBEE_START_DELIMITER;
    outbuf[1] = len/256;
    outbuf[2] = len%256;
    outbuf[3] = 0x10;  //frame type
...
data_ptr_t xbee_rx_data_read(String xbee_data,int len) {
	...
	payload_str = wframe_get_data_between_del(wframe,len-13, 4,fields+4,'#');
...
data_ptr_t xbee_exrx_data_read(String xbee_data,int len) {
	...
	map_ptr_t payload_map = NULL;
	int fields = *(wframe+4); //the fifth char, expressed as 0x03(char with asc value 3), so no need to convert (from asc) to number
	payload_str = wframe_get_data_between_del(wframe,len-19, 4,fields+4,'#');
	payload_map = str_to_map(payload_str,'#',':');
	printf("%d %d %d\n",wframe[0],wframe[1],fields);
...
void tty_serial_config(int fds) {
	...
	//input option:ignore parity check, map CR(Enter,carriage return,'\r' 10) to NL(new line, '\n', 10)
	//tio_new.c_iflag |= (IGNPAR | ICRNL);
	//don't map CR and NL
	tio_new.c_iflag |= IGNPAR;
	tio_new.c_iflag &= ~(INLCR | ICRNL | IGNCR);
	//c_cc数组的VSTART和VSTOP元素被设定成DC1和DC3
	//代表ASCII标准的XON和XOFF字符，如果在传输这两个字符的时候就传不过去
	//需要把软件流控制屏蔽
...
int tty_serial_read(int fds, String tbuf) {
	...
	if(inbuf[0]) memset(tbuf,0,MAX_FRAME_SIZE);
	
	ssize_t ret;
	int i,bytes=3,leng = 3;
	...
	if(tbuf[0]=='~') {
		leng = tbuf[1]*256 + tbuf[2] +1;  //including the checksum bit
		printf("received data section length: %d\n",leng);
		...
		do {
			...
			nr_escapes = 0;
			for(i=bytes-leng;i<bytes;i++) {  
				if(tbuf[i]==0x7D) {
					printf("the byte after escape: %d\n",tbuf[i+1]);
					//int c = tbuf[1+i];
		...
		bytes = xbee_frame_descape(tbuf,bytes);
			
		if(xbee_set_checksum(&tbuf[3],bytes-3)!=0) {
			printf("%s\n","checksum error...");
			memset(tbuf,0,bytes);
			return 0;
		}
		...
		leng = bytes;
		fbuf = tbuf;
		int dlogfds = open("./desclog.txt", O_CREAT | O_RDWR | O_APPEND);
...
String wframe_get_data_at_del( String pkt, int len, int n,  char del) {
	...
	if(i+1<j-1) return str_get_between_index(pkt, i+1, j-1);
		...
		String c = (String)malloc(2);
		*c = *(pkt+i+1);
		*(c+1) = 0;
		return c;
	} else {               //the 2 dels are next to each other
		String c = (String)malloc(1);
		*c = 0;
		return c;
	}
}
...
int wframe_assemble(Byte frame_type, Byte fields, String serial_id,Byte seq,map_ptr_t payload,String* wframe) {
	...
	String start = malloc(6); //first 5 bytes
	*start = 60;
	*(start+1) = 61;
	*(start+2) = 62;
	*(start+3) = frame_type;
...
long power(Byte a, unsigned char b) {
	unsigned char i;
	long pow = 1;
...
String str_get_between_index( String str, int bn, int en) {
	if(!str || bn<0 || bn>en) {
	...
	String data = (String)malloc(en-bn+2);
	int i;
	for(i=bn; i<=en; i++) {
		*(data+i-bn) = *(str+i);
	}
...
int str_split( String str, char del,String** retArray) {
	...
	str_array = (String*)malloc(size);
	//printf("%d %d\n",j,size);
	for(i=0; i<j; i++) {  //fill the array row by row
		int k;
		if(i==0) {
			*(str_array+i) = (String)malloc(index[i]+1);
			for(k=0; k<index[i]; k++) {
				*(*(str_array+i)+k) = *(str+k);
			}
			...
			*(str_array+i) = (String)malloc(index[i]-index[i-1]);
...
String str_multi_cat(int num,  char del, ...) {
	String str = (String)malloc(MAX_FRAME_SIZE);
    va_list argmt;
    va_start(argmt,del);
    int i,j,index=0;
    for(i=0; i<num; i++) {
        String sf = (String)va_arg(argmt,String);
...
int map_cat(map_ptr_t map,  char fdel,  char sdel, String* str) {
	...
	String* sensors = (String*)malloc((fields+1)*sizeof(String));
	int i,len=0;
	for(i=0; i<fields; i++) {
		len += map_to_str(map+i,sdel,&(*(sensor+i)));
	}
	*(sensors+fields) = NULL;
	*str = (String)str_multi_cat(fields,);
	*/
	String str_temp = (String)malloc(MAX_FRAME_SIZE); //initialize the memory block so str_length() returns 0
...
String map_get_value(map_ptr_t map,String name) {
...
int main() {
    ...
    while(bytes<=0) {
		...
		printf("received data length: %d %d %d %d %d\n",bytes,inbuf[16],inbuf[17],inbuf[18],inbuf[89]);
		sleep(3);
	}
    ...
    //Byte mac_addr[9] = {0x00,0x13,0xA2,0x00,0x40,0x7A,0x24,0x7A};
    Byte mac_addr[9] = {0x00,0x13,0xA2,0x00,0x40,0x79,0x5B,0xCE};
    Byte ntw_addr[3] = {0xFF,0xFE};
    String RFData = NULL;
    ...
    printf("frame length: %d %d ",bytes,len);
	...
	int bytes = xbee_localAT_frame_assemble("NP",'Q');
    int len =bytes;
	//bytes = xbee_frame_escape(bytes);	
