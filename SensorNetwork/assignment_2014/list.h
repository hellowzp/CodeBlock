...
// Returns an index to the first element in 'list' containing 'data'.  If 'data' is not found in 'list', -1 is returned.
  ...
  list_ptr_t list_insert_sorted( list_ptr_t list, data_ptr_t data, int (*compare_func)(const void*, const void*) );
  // Inserts a new element containing 'data' in the sorted 'list' and returns a pointer to the new list. The 'list' must be sorted before calling this function. The sorting is done in ascending order according to a comparison function pointed to by compare_func, which is called with two arguments that point to the objects being compared. The comparison function must return an integer less than, equal to, or greater than zero if the first argument is considered to be respectively  less than, equal to, or greater than the second.  If two members compare as equal, their order in the sorted array is undefined.
