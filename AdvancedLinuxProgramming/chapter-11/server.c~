...
static void handle_connection (int connection_fd)
{
  ...
  if (bytes_read > 0) {
	...
	fprintf(stderr,"%s %d bytes, last byte: %d\n","http request read...",bytes_read,buffer[bytes_read-1]);
    ...
    /* The client may send various header information following the
       ...
       delimiter.  */
    while (strstr (buffer, "\r\n\r\n") == NULL)  //the complete http request is not finished yet
      bytes_read = read (connection_fd, buffer, sizeof (buffer));  //continue reading
