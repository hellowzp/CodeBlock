...
int list_size( list_ptr_t list ){
 ...
 list_ptr_t list_insert_at_index( list_ptr_t list, data_ptr_t data, int index){
     ...
     assert(temp);  
	 temp->data=*data;
	 if(list->data==0){    //not initialized
        temp->next=NULL;
        temp->prev=NULL;
        return temp;	
     } 	  
...
list_ptr_t list_insert_sorted( list_ptr_t list, data_ptr_t data ){
	...
	for(tempCurrent=list;tempCurrent->next!=NULL;tempCurrent=tempCurrent->next){
	                      //sorted from small till big
