...
int main( void )
{
  ...
  while(1 ) {	
    ...
    if(P_PID == 0){
		...
		printf("You want to save %s",file_name);
		...
		//begin the process to write in a file
    ...
    if (P_PID == 0){
		...
		while ((WAIT_PID = wait(&status)) > 0)
   		{
        		printf("STATE of %d was %d (%s)\n", (int)WAIT_PID, status,(status > 0) ? "EXIT" : "STILL RUNNING");
   		}
		
		
	}
	
    //sending finished ,close the client 
    tcp_close( &client );
  }
