...
//----------------------------------------------------------------------------//
{
	...
	for (i = 0; i < list_size(list) ; i++)
	{
		...
		if (list->funcs->compare_data(data, _data) <= 0)
		{
	...
	else
	{
		for (i = 0; i < list_size(list)-1; i++)
		{
			...
			for (j = i + 1; j < list_size(list); j++ )
			{
				...
				if ( list->funcs->compare_data(ptr_1->data, ptr_2->data) > 0) //if data at index i is bigger than at j then exchange
				{
					data_med = ptr_1->data;
...
//----------------------------------------------------------------------------//
{
	...
	{	// if data match, list will be pointing to the matching element
		if( list->funcs->compare_data(list->data, data) == 0)
		{
			match = 1;
			break;
		}
...
//----------------------------------------------------------------------------//
{
	...
	if (match)
	{
		...
		else
		{
			...
			{
				ptr = list->next;
				ptr->funcs->destory_data(list->data);
				free(list);
				ptr->previous = NULL;
				err = NONE;
				return ptr;
...
//----------------------------------------------------------------------------//
{
	...
	if (reference == NULL)
	{
		...
		{
			list = list->previous;
			list->funcs->destory_data(list->next->data);
			free(list->next);
			list->next = NULL;
			err = NONE;
			return list;
	...
	else
	{
		...
		if (exist_ref)
		{
			...
			else
			{
				...
				{
					ptr = reference->next;
					ptr->funcs->destory_data(reference->data);
					free(reference);
					ptr->previous =NULL;
					err = NONE;
					return ptr;
...
//----------------------------------------------------------------------------//
{
	...
	new_element->data = list->funcs->deep_copy(data);
...
//----------------------------------------------------------------------------//
{
	...
	new_element = (list_ptr_t)malloc(sizeof(list_t)); 	//allocate the new element
	new_element->funcs = list->funcs;
	new_element->data = list->funcs->deep_copy(data);	//deep copy the data into the new element
	int i;
...
//----------------------------------------------------------------------------//
{
	...
	callback_funcs * funcs = list->funcs;		// define a pointer pointing to malloc funcs that we are going to use
					
	list = list_get_first_reference(list);		// move the pointer to the begainning of list
	while(list->next != NULL)
	{
		list = list->next;
		funcs->destory_data(list->previous->data);  	//free the data of the previous element
		free(list->previous);
	}
	
	funcs->destory_data(list->data);		//free the data in last element
...
//----------------------------------------------------------------------------//
list_ptr_t list_alloc (destory_data_func *destory_data, compare_data_func *compare_data, deep_copy_func *deep_copy, data_ptr_t data)
//----------------------------------------------------------------------------//
{
	list_ptr_t list = (list_ptr_t)malloc(sizeof(list_t));
	list->funcs = (callback_funcs *)malloc(sizeof(callback_funcs));
	list->funcs->destory_data = destory_data;
	list->funcs->compare_data = compare_data;
	list->funcs->deep_copy = deep_copy;
	list->next = NULL;
	list->previous = NULL;
	list->data = deep_copy(data);	
