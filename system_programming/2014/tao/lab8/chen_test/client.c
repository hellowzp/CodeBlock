...
int main( void ) {
  ...
  tcp_send( server, (void *)buffer, strlen(buffer)+1 );
  bzero(buffer,sizeof(buffer));
  
  //a loop used to recieve the feedback from server
  while(buffer == NULL){
  
  ret = tcp_receive (server, buffer, BUFSIZE);
  }
  ...
  while(fread(msg,10,sizeof(char),fp)==1&&( (ret = tcp_receive (server, buffer, BUFSIZE)) > 0 )){
	   
	printf("writing %s",msg);
	// send msg to server
	tcp_send( server, (void *)msg, strlen(msg)+1 );
	//sleep(1);
