...
int main(int argc,char *argv[])
{
    ...
    else if(argc==1)
    {
      ...
      //waiting for input
       ...
       flag1= Num/abs(Num);     //number of column need to print
       FullRow = abs(Num)/10;
       flag2 = abs(Num)%10;     //the number of the last row
 
       printf("Address          Bytes                            Chars\n");
       printf("--------         ----------------------------     -------\n");
        
        
        //begin to print from row to column
	for (i = 0; i < FullRow; i++)
	{
	   printf("%p\t",Q);
	   for (j = 0; j < a; j++)
	...
	if (flag2 != 0)    //if the last row is not full then go to print
	{
		printf("%p\t",Q);
		for(i = 0; i < a; i++)
		{
