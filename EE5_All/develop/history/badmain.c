...
mqtt_ptr_t xbee_rx_data_read(String xbee_data,int len) {
	...
	mqtt->len = len-j-3;
	mqtt->msg = (void*)str_get_between_index(xbee_data,j+3,len-1);	
	printf("%s\n",(char*)(mqtt->msg));
...
mqtt_ptr_t xbee_exrx_data_read(String xbee_data,int len) {
	...
	//should avoid writing at the same time here
	int bytes = xbee_remoteAT_frame_assemble(mac_addr,ntw_addr,0x02,"AP",'Q'); //apply changes and set API mode 2
	tty_serial_write(tty_fds,outbuf,bytes);
